package hello.blog.security.oauth;
...
//            CustomUserDetails customUserDetails = new CustomUserDetails(user.getUserName(), user.getPassword(), user.getRole().stream().map(role -> role.getRoleName().name()).collect(Collectors.toList()));
//
...
//
//            response.sendRedirect("/trending");
//
